...
public class ShaderLoader {
      ...
      if (this.programmId == -1) {
         ...
         try {
            ...
            if (this.vertexShaderId == -1) {
               ...
               fragmentShader = ParticleRenderUtils.getShaderCode(new InputStreamReader(in));
               this.vertexShaderId = ParticleRenderUtils.createShader(fragmentShader, 35633);
            }

            if (this.fragmentShaderId == -1) {
               in = this.getClass().getResourceAsStream("fragment/" + this.fragmenShaderFileName);
               fragmentShader = ParticleRenderUtils.getShaderCode(new InputStreamReader(in));
               this.fragmentShaderId = ParticleRenderUtils.createShader(fragmentShader, 35632);
            }
         ...
         if (this.programmId != -1) {
            ...
            this.resolutionUniformId = ARBShaderObjects.glGetUniformLocationARB(this.programmId, (CharSequence)"resolution");
            this.timeUniformID = ARBShaderObjects.glGetUniformLocationARB(this.programmId, (CharSequence)"timeHelper");
            this.mouseUniformId = ARBShaderObjects.glGetUniformLocationARB(this.programmId, (CharSequence)"mouse");
            this.diffuseSamperUniformID = ARBShaderObjects.glGetUniformLocationARB(this.programmId, (CharSequence)"diffuseSamper");
            this.texelUniformId = ARBShaderObjects.glGetUniformLocationARB(this.programmId, (CharSequence)"texel");
         }
      }
   ...
   public ShaderLoader update() {
      if (this.fboID != -1 && this.renderBufferID != -1 && this.programmId != -1) {
         ...
         this.time = (float)((double)this.time + (double)ParticleRenderUtils.delta * 0.7);
         ...
         GL11.glVertex2d(0.0, height / 2.0);
         GL11.glTexCoord2d(1.0, 0.0);
         GL11.glVertex2d(width / 2.0, height / 2.0);
         GL11.glTexCoord2d(1.0, 0.0);
         GL11.glVertex2d(width / 2.0, height / 2.0);
         GL11.glTexCoord2d(1.0, 1.0);
         GL11.glVertex2d(width / 2.0, 0.0);
         GL11.glTexCoord2d(0.0, 1.0);
         GL11.glVertex2d(0.0, 0.0);
         GL11.glEnd();
         ARBShaderObjects.glUseProgramObjectARB(0);
